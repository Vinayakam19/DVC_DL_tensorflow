...
def get_prepare_model(model, CLASSES, freeze_all, freeze_till, learning_rate):
    if freeze_all:
    ...
    predictions = tf.keras.layers.Dense(CLASSES, activation='softmax')(flatten_in)
    
    full_model = tf.keras.models.Model(inputs=model.input, outputs=predictions)
   
    full_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate),loss=tf.keras.losses.categorical_crossentropy,metrics=['accuracy'])
